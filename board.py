...
def check_blocks(board, player):
    ...
    for i, row in enumerate(horizontal):
        ...
        for j, item in enumerate(row):
            if item:
                ...
                index[1] = j
        if len(pair) == 2 and pair[0] == pair[1] == player['human']:
            index[0] = i
            return True, tuple(index)

    for i, column in enumerate(vertical):
        pair = []
    ...
    pair = []
    for i, item in enumerate(left_diagonal):
        if item:
            pair.append(item)
        else:
            index[1] = i
    ...
    pair = []
    for i, item in enumerate(right_diagonal):
        if item:
            pair.append(item)
        else:
            index[0] = i
